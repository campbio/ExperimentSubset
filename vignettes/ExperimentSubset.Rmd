...
The `storeSubsetAssay` method should be used when a subset `assay` needs to be stored
...
has no assays stored. The `storeSubsetAssay` method however, is used to store an
...
`storeSubsetAssay` method can be used to directly create a subset and then store an
